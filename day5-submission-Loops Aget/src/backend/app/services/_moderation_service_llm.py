"""
Moderation Service — LLM API approach (zero ML dependencies)

Pipeline: RuleEngine (harm) → LLM API (tech relevance) → DecisionEngine.
Same public interface as the ensemble/ollama services: exposes a
`ModerationService` class with `moderate_post(post_id, text, image_path)`.

NOTE: This approach does NOT do image analysis (no CLIP/NSFW models), which
is what keeps it torch-free. `image_path` is accepted for interface
compatibility but not visually inspected — nsfw_score stays 0.
"""
...
from app.ml.llm_moderator import get_llm_moderator
from app.db.mongodb import post_repository

logger = logging.getLogger(__name__)

print("🔥 MODERATION SERVICE LOADING (LLM API approach)...")
sys.stdout.flush()
...
class ModerationService:
    def __init__(self):
        ...
        logger.info("✅ Moderation service initialized (LLM API)")
        print("🔥 MODERATION SERVICE INITIALIZED (LLM API)")
        sys.stdout.flush()
    ...
    def _is_borderline(self, decision_input: dict) -> tuple[bool, list]:
        ...
        Triggers when tech relevance is in the review zone OR models disagree.
        """
        reasons = []
    ...
    async def moderate_post(
        ...
        try:
            # Step 1: Rule-based checks (handles ALL harm detection)
            ...
            # Step 3: Mixing detection + tech relevance (rule-based)
            ...
            if rule_results.get('rule_score', 0) > 0.8:
                logger.warning("🛑 EARLY BLOCK by rule engine")
                return {"post_id": post_id, "allowed": False, "results": {"block_reason": "rule_engine"}}

            # Step 4: LLM API tech-relevance analysis
            try:
                print("🔥 Calling LLM API...")
                sys.stdout.flush()
                model = get_llm_moderator()
                text_results = await self._run_sync(model.analyze, text)
                print(f"🔥 LLM result: tech={text_results.get('scores', {}).get('tech_relevance', 'N/A')}")
                sys.stdout.flush()
                logger.info("✅ LLM text analysis complete")
            ...
            # Step 5: Image analysis intentionally omitted (torch-free approach).
            nsfw_score = 0.0
            ...
            # Borderline check — flag for human review before final decision
